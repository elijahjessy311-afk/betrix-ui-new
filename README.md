...
└─────────────┬───────────────────────────┬───────────────────┘
      ...
      │ ┌──────────────────────────────┤ │
      │ │ • HTTP Endpoints             │ │
      │ │ • Free Data Sources (/live,  │─┼─▶ Broadcast prefetch
      │ │   /standings, /news, etc.)   │ │   events
      ...
      │   Worker Process               │ │
      │ ┌──────────────────────────────┤ │
      │ │ • BRPOPLPUSH Queue Handler   │ │
      │ │ • AI Composite Chain         │ │
      │ │ • Prefetch Scheduler         │─┼─▶ Pub/Sub Events
      │ │ • Handler Router             │ │
      │ │ • Redis Subscriber (observer)│ │
      └─┬──────────────────────────────┘ │
        │                                 │
        │ All read/write via Redis        │
        │                                 │
      ┌─▼──────────────────────────────┐ │
      │    Redis (ioredis)              │ │
      │ ┌──────────────────────────────┤ │
      ...
      │ │  • prefetch:openligadb:*      │ │
      │ │  • prefetch:scorebat:*        │ │
      │ │  • prefetch:footballdata:*    │ │
      │ │                               │ │
      │ │ State:                        │ │
      │ │  • prefetch:failures:*        │ │
      │ │  • prefetch:next:* (backoff)  │ │
      │ │  • ai:active (provider)       │ │
      │ │  • worker:heartbeat           │ │
      │ │  • system:logs                │ │
      │ │  • context:*:history          │ │
...
# ✔ CacheService - rate limiting within bounds
# ✔ CacheService - rate limiting exceeds bounds
# ✔ CacheService - handles null/undefined values
# ✔ CacheService - serialization/deserialization
# ✔ OpenLigaDBService instantiates without cache
# ✔ OpenLigaDBService instantiates with CacheService
# ✔ RSSAggregator instantiates without cache
# ✔ RSSAggregator instantiates with cache
# ✔ Smoke tests
```
