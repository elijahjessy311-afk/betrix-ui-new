...
Provide a 2-3 sentence expert analysis on why this prediction has high confidence, considering form, head-to-head, and statistics.
  ...
  formatForTelegram(analysis) {
    ...
    // Safe extraction of league name: handle non-string homeTeam
    let leagueName = 'Unknown';
    if (analysis.matchData && analysis.matchData.homeTeam) {
      try {
        const ht = String(analysis.matchData.homeTeam);
        leagueName = ht.split('/')[0] || 'Unknown';
      } catch (e) {
        leagueName = 'Unknown';
      }
    }
