...
export async function reconcileWithLipana(opts = {}) {
  const { pool, telegram, thresholdMinutes = 5, limit = 200, adminId = null } = opts;
  ...
  for (const r of rows) {
    ...
    try {
      ...
      if (status && String(status).toLowerCase() === 'success') {
        ...
        if (telegram && adminId) {
          try { await telegram.sendMessage(adminId, `✅ Reconciled payment ${providerId} -> marked SUCCESS (${updated.length} rows)`); } catch(e) { void e; }
        }
        ...
        if (telegram && adminId) {
          try { await telegram.sendMessage(adminId, `❌ Reconciled payment ${providerId} -> marked FAILED (${updated.length} rows)`); } catch(e) { void e; }
        }
      } else {
