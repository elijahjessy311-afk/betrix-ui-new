...
## Quick Staging Checklist (copy-paste)

1. Create a file named `.env` on the staging host with the values from `.env.example`.
2. Ensure `REDIS_URL` points to the staging Redis and is reachable from the app host.
3. Configure `PAYPAL_*` sandbox credentials and `PAYPAL_WEBHOOK_ID` in the platform secrets.
4. Deploy the `main` branch to staging (Render/Heroku/your provider).
5. Run the basic smoke checks (PowerShell):

```powershell
# load env from .env (PowerShell example using dot-sourcing of env variables file)
Get-Content .env | ForEach-Object { $parts = $_ -split '='; if ($parts.Length -ge 2) { $name = $parts[0].Trim(); $value = $parts[1..($parts.Length-1)] -join '='; $env:$name = $value } }

# run smoke tests
node --test tests/*.js

# run the payment harness (creates test orders only)
node scripts/test-payment-harness.js
```

See `.env.example` for placeholders and required keys.
