...
async function runTests() {
  ...
  try {
    ...
    const order1 = await createPaymentOrder(redis, userId, tier, 'SAFARICOM_TILL', region);
  ...
  try {
    ...
    const order2 = await createPaymentOrder(redis, userId + 1, tier, 'till', region);
  ...
  try {
    ...
    const order3 = await createPaymentOrder(redis, userId + 2, tier, 'mpesa_stk', region);
