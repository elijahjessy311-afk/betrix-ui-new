...
const redis = new Redis({
  ...
  lazyConnect: true,
  // keep retries low in test environment to avoid long hangs when Redis is not present
  maxRetriesPerRequest: 1,
  // disable aggressive reconnects for tests
  reconnectOnError: () => false
});

// Prevent unhandled error events from ioredis when Redis is not available in local test env
redis.on('error', (err) => {
  console.warn('⚠️  [ioredis] Ignoring Redis error in test:', err && err.message ? err.message : err);
});
